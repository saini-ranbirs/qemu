    ...
    if (s->have_rpmi) {
        ...
        for (i = 0; i < rpmi_xports; i++) {
            ...
            create_fdt_rpmi_nodes(s, i, shm_base, db_base,
                                  ...
                                  db_sz);
            mmshm_base = s->memmap[VIRT_MM_SHMEM].base;
            mmshm_size = s->memmap[VIRT_MM_SHMEM].size;
            riscv_rpmi_create(db_base, shm_base, shm_sz,
                              a2preq_qsz, p2areq_qsz,
                              fcm_base, fcm_sz,
                              harts_mask, soc_xport_type,
                              mmshm_base, mmshm_size, ms);
        }
    } else {
