...
static rpmi_uint64_t fn_get_variable(struct rpmi_mm_efi *efi,
				     ...
				     rpmi_uint32_t payload_size)
{
	...
	return efi->ops->get_variable(efi->ops_priv, comm_hdr->data,
				      payload_size);
}
...
static rpmi_uint64_t fn_get_next_var_name(struct rpmi_mm_efi *efi,
					  struct efi_var_comm_header *comm_hdr,
					  rpmi_uint32_t payload_size)
{
	rpmi_uint64_t status;
	...
	return efi->ops->get_next_variable_name(efi->ops_priv, comm_hdr->data,
						payload_size);
}

static rpmi_uint64_t fn_set_variable(struct rpmi_mm_efi *efi,
				     struct efi_var_comm_header *comm_hdr,
				     rpmi_uint32_t payload_size)
{
	rpmi_uint64_t status;
	...
	return efi->ops->set_variable(efi->ops_priv, comm_hdr->data,
				      payload_size);
}
...
static enum rpmi_error efi_var_fn_handler(struct rpmi_mm_efi *efi,
					  void *comm_buf, rpmi_uint64_t bufsize)
{
	...
	case EFI_VAR_FN_GET_VARIABLE:
		DPRINTF("Processing %s efi_calls_counter %u",
			...
			++efi_calls_counter);
		status = fn_get_variable(efi, var_comm_hdr, payload_size);
		break;
	...
	case EFI_VAR_FN_GET_NEXT_VARIABLE_NAME:
		DPRINTF("Processing %s efi_calls_counter %u",
			get_var_fn_string(var_comm_hdr->function),
			++efi_calls_counter);
		status = fn_get_next_var_name(efi, var_comm_hdr, payload_size);
		break;
	...
	case EFI_VAR_FN_SET_VARIABLE:
		DPRINTF("Processing %s efi_calls_counter %u",
			get_var_fn_string(var_comm_hdr->function),
			++efi_calls_counter);
		status = fn_set_variable(efi, var_comm_hdr, payload_size);
		break;
...
enum rpmi_error mm_efi_communicate(rpmi_uint64_t shmem_addr,
				   ...
				   rpmi_uint8_t *rsp_data)
{
	...
	rpmi_uint32_t *rsp = (void *)rsp_data;
	...
	mm_addr = shmem_addr + mmc_req->idata_off;
	...
	case MM_EFI_VAR_PROTOCOL_GUID:
		...
		status =
		    efi_var_fn_handler(&mmipops->inst_efi, &msg->data, msg_len);
