...
extern rpmi_uint64_t get_mm_group_shmem_address(struct rpmi_service_group *grp);
...
static enum rpmi_error efi_var_policy_handler(struct rpmi_shmem *mm_shmem,
					      ...
					      void *priv_data)
{
	enum mm_efi_header_guid guid_name = (enum mm_efi_header_guid)priv_data;
...
enum rpmi_error register_rpmi_mm_efi_service(struct rpmi_service_group *group,
					     struct rpmi_mm_efi *ipefi)
{
	...
	struct rpmi_mm_service efi_srvlist[] = {
		[0] {
			...
			.cbfn_p = efi_var_protocol_handler,
			.priv_data = (void *)MM_EFI_VAR_PROTOCOL_GUID,
		},
		[1] {
			.guid = MM_EFI_VAR_POLICY_GUID_DATA,
			.cbfn_p = efi_var_policy_handler,
			.priv_data = (void *)MM_EFI_VAR_POLICY_GUID,
		},
		[2] {
			.guid = MM_EFI_END_OF_DXE_GUID_DATA,
			.cbfn_p = efi_dummy_handler,
			.priv_data = (void *)MM_EFI_END_OF_DXE_GUID,
		},
		[3] {
			.guid = MM_EFI_READY_TO_BOOT_GUID_DATA,
			.cbfn_p = efi_dummy_handler,
			.priv_data = (void *)MM_EFI_READY_TO_BOOT_GUID,
		},
		[4] {
			.guid = MM_EFI_EXIT_BOOT_SVC_GUID_DATA,
			.cbfn_p = efi_dummy_handler,
			.priv_data = (void *)MM_EFI_EXIT_BOOT_SVC_GUID,
		},
	};

	/* Allocate MM EFI platform operations struct */
	if (ipefi) {
