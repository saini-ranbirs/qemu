...
#include "EDK2_VariableParsing.h"
...
static int set_variable_to_variable_store(SMM_VARIABLE_COMMUNICATE_ACCESS_VARIABLE *smm_var_hdr)
{
	EFI_STATUS              Status;
...
static inline int fxn_set_variable(rpmi_uint8_t *comm_hdr_data,
				   rpmi_uint32_t bfr_payload_size)
{
	SMM_VARIABLE_COMMUNICATE_ACCESS_VARIABLE *smm_var_hdr;
				    ...
				    rpmi_uint32_t *comm_bfr_size)
{
	...
	case SMM_VARIABLE_FUNCTION_SET_VARIABLE:
		memcpy(m_var_bfr_payload, smm_var_comm_hdr->Data,
		       comm_bfr_payload_size);
		status = fxn_set_variable(smm_var_comm_hdr->Data,
					  comm_bfr_payload_size);
		if (status == EFI_SUCCESS)
			memcpy(smm_var_comm_hdr->Data, m_var_bfr_payload,
			       comm_bfr_payload_size);
